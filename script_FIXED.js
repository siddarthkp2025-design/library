...
async function handleAddBook(e) {
    ...
    try {
        ...
        // Clear return book list - let user enter member ID first
        const returnContainer = document.getElementById('availableBooksListReturn');
        if (returnContainer) {
            returnContainer.innerHTML = '<div class="book-item">Enter a valid member ID to see borrowed books</div>';
        }
...
async function handleReturnBook(e) {
    ...
    try {
        ...
        if (result > 0) {
            ...
            // Make the borrowed books container completely empty FIRST
            const returnContainer = document.getElementById('availableBooksListReturn');
            if (returnContainer) {
                returnContainer.innerHTML = '';
            }
            
            // Clear input field AFTER clearing container to avoid triggering listener
